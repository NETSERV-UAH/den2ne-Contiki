...
#else
	#define IOTORII_STATISTICS1_TIME 5
#endif
...
#else
	#define IOTORII_SHARE_START_TIME 5 //10 POR DEFECTO -> AUMENTAR EN CASO DE MAS DE 30 NODOS
#endif
...
uint32_t iotorii_extract_timestamp (void) //SE EXTRAE EL TIMESTAMP A PARTIR DEL PAQUETE RECIBIDO
{
	...
	//SE LEE EL PAYLOAD
	memcpy(&timestamp, packetbuf_ptr, sizeof(uint32_t)); //COPIA EL TIMESATMP
...
static void iotorii_handle_statistic_timer ()
{
	...
	if(number_of_neighbours!=0){
		if (start_share == 0 && msg_share_on == 0)
		{
			...
			printf("//INFO STATISTICS// n_hello: %d, n_sethlmac: %d\n\r", number_of_hello_messages, number_of_sethlmac_messages);
			...
			if (!number_of_neighbours_flag)
			{
				...
				if (start_load == 0 && sent_no_edge == 0) //NO SE HA INICIADO EL ENVÍO DE MENSAJES TODAVÍA
					ctimer_set(&load_timer, (random_rand() % IOTORII_LOAD_START_TIME) * CLOCK_SECOND, iotorii_handle_load_timer, NULL); 
			}
			else
			{
				printf("no es edge\n\r");
...
static void input_packet (void) //SE RECIBE UN PAQUETE 
{
	...
	else 
	{
		...
		if (duplicate) //SI LO ESTÁ, SE BORRA
		{
			...
			LOG_WARN_(", seqno %u\n\r", packetbuf_attr(PACKETBUF_ATTR_MAC_SEQNO));
		} 
		else 
		{
		...
		if (!duplicate)  //SI NO ESTÁ DUPLICADO
		{
			...
			LOG_INFO_(", seqno %u, len %u\n\r", packetbuf_attr(PACKETBUF_ATTR_MAC_SEQNO), packetbuf_datalen());
...
void list_this_node_entry (payload_entry_t *a, hlmacaddr_t *addr)
{
	...
	{		
		...
		printf("[addr: %s | top_addr: %s | payload/hops: %d | node_load: %d]\n\r", new_address->str_addr, new_address->str_top_addr, new_address->nhops, new_address->load);
		aux = aux->next;
	}
}
